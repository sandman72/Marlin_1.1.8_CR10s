...
static void lcd_implementation_status_screen() {
  ...
  if (PAGE_UNDER(STATUS_SCREENHEIGHT + 1)) {
    ...
    u8g.drawBitmapP(1, 1, (STATUS_SCREENWIDTH + 7) / 8, STATUS_SCREENHEIGHT,
      #if HAS_FAN0
  ...
  if (PAGE_UNDER(28)) {
    // Extruder
    HOTEND_LOOP() _draw_heater_status(39 + e * 25, e, blink);

    // Heated bed
    #if HOTENDS < 4 && HAS_TEMP_BED
      _draw_heater_status(74, -1, blink);
    #endif
  ...
  // Alive Dot
  //
  if (blink) u8g.setColorIndex(1); else u8g.setColorIndex(0); // Set color for the alive dot
